...
Bodyguard lets a user paste a public website address and get a risk report before opening it. The backend checks its public HTTP response and HTML, checks the URL against Google Safe Browsing when configured, and can ask Gemini to review the public page using URL Context. Both API keys stay on the server.
...
3. Copy `.env.example` to `.env`. Add your `SAFE_BROWSING_API_KEY` and `GEMINI_API_KEY`; either check can be omitted by leaving its key blank.
...
This version fetches public HTML but does not execute the target site's JavaScript or render its layout. It cannot see traps created only after scripts run, external stylesheets, canvas or image-only content, or behavior requiring a browser session. Google Safe Browsing checks run only when `SAFE_BROWSING_API_KEY` is configured. Gemini can retrieve the public URL through URL Context when `GEMINI_API_KEY` is configured; its assessment is heuristic, not proof of safety. Bodyguard does not query VirusTotal. Do not treat results as a substitute for browser protections.

Scanning sends the address to the Bodyguard server, which fetches the public page and sends the URL to Google Safe Browsing and Gemini when configured. Gemini may independently retrieve the public URL using URL Context; the HTML fetched by Bodyguard is not sent to Gemini. Since the URL string can include its path and query parameters, do not scan links containing private tokens or other secrets. The server refuses private/reserved IP ranges, nonstandard ports, userinfo URLs, and redirects into private networks. It caps response size, request time, scan concurrency, and scan frequency. The extension observes page URLs locally and contacts the backend only after the user clicks its check button.
...
- `.env.example` — server-side Google Safe Browsing, Gemini, and listener configuration.
