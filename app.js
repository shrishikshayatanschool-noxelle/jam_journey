(() => {
  ...
  function normalizeWebsiteInput(raw) {
    const value = raw.trim().replace(/^<(.+)>$/, '$1').replace(/^(["'`])(.*)\1$/, '$2');
    const webScheme = value.match(/^(https?):\/*(.*)$/i);
    if (webScheme) return `${webScheme[1].toLowerCase()}://${webScheme[2].replace(/^\/+/, '')}`;
  ...
  function renderReport(report) {
    ...
    if (report.aiAssessment) {
      ...
      heading.textContent = report.aiAssessment.status === 'complete'
        ? `Gemini website review · ${report.aiAssessment.verdict.toUpperCase()} · ${report.aiAssessment.confidence}% confidence`
        : 'Gemini website review';
      const detail = document.createElement('p');
    ...
    try {
      ...
      showError(cause.message === 'Failed to fetch'
        ? (['localhost', '127.0.0.1'].includes(window.location.hostname)
          ? 'Could not reach the local scanner. In the project folder, run “node server.mjs” and keep that terminal open.'
          : 'Could not reach the Bodyguard scanner service. Please try again in a moment; the site owner may need to restart the scanner.')
  ...
  form.addEventListener('submit', (event) => {
    ...
    if (!rawValue) { showError('Paste a website address to scan.'); input.focus(); return; }
    const value = normalizeWebsiteInput(rawValue);
    input.value = value;
    scan(value);
  });
  input.addEventListener('input', () => { error.hidden = true; input.removeAttribute('aria-invalid'); });
  document.querySelectorAll('[data-fill-url]').forEach((button) => {
    button.addEventListener('click', () => {
      input.value = button.dataset.fillUrl;
      error.hidden = true;
      input.removeAttribute('aria-invalid');
      document.querySelectorAll('.test-card').forEach((card) => card.classList.toggle('is-used', card.contains(button)));
      const testFeedback = $('#test-feedback');
      if (testFeedback && button.closest('.test-card')) {
        const selectedHost = new URL(button.dataset.fillUrl).hostname;
        testFeedback.textContent = `${selectedHost} is ready. Now press “Check link” to run the scan.`;
      }
  ...
  function showRating(value) {
    ratingButtons.forEach((button) => {
      ...
      button.setAttribute('aria-pressed', String(Number(button.dataset.rating) === value));
    });
    if (ratingFeedback) ratingFeedback.textContent = value
      ? `Thanks — your ${value}-star rating is saved in this browser.`
      : 'Choose a rating';
  }
  showRating(Number(localStorage.getItem('bodyguard-rating') || 0));
